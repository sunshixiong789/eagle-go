...
// Compare generated Ent metadata with goose's real PostgreSQL schema. SQL-only
...
func TestEntMatchesGoose(t *testing.T) {
	...
	pg, err := testkit.StartPostgres("schema-parity", "eagle_schema_test")
	if err != nil {
		t.Fatal(err)
	}
...
func assertColumns(t *testing.T, db *sql.DB, table *schema.Table) {
	t.Helper()
	rows, err := db.QueryContext(context.Background(), `SELECT column_name, data_type, COALESCE(character_maximum_length,0), is_nullable FROM information_schema.columns WHERE table_schema = 'public' AND table_name = $1`, table.Name)
	if err != nil {
		t.Fatal(err)
	}
	defer func() { _ = rows.Close() }()
	...
	for _, col := range table.Columns {
		...
		if wantType == "" {
			t.Fatalf("add explicit PostgreSQL mapping for %s.%s type %v", table.Name, col.Name, col.Type)
		}
		got, ok := actual[col.Name]
		if !ok || got.typ != wantType || got.nullable != col.Nullable || (col.Type == field.TypeString && got.size != col.Size) {
			t.Errorf("%s: goose=%+v ent=(%s,size=%d,nullable=%t)", col.Name, got, wantType, col.Size, col.Nullable)
		}
	}
}

func assertIndexes(t *testing.T, db *sql.DB, table *schema.Table) {
	t.Helper()
	rows, err := db.QueryContext(context.Background(), `SELECT i.indisunique, i.indisprimary, string_agg(a.attname, ',' ORDER BY k.ordinality)
 ...
 GROUP BY i.indexrelid,i.indisunique,i.indisprimary`, table.Name)
	if err != nil {
		t.Fatal(err)
	}
	defer func() { _ = rows.Close() }()
