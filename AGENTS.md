...
- 依赖：`service → biz → domain ← data`。`domain`/`biz` 不碰 Kratos、proto、Ent、Redis、Casbin。只有 `data` 碰 Ent。`pkg/` 不 import `app/`。
- 有不变量才上聚合根。字典保持贫血。biz 禁止复制 data 锁内检查（父节点/成环/子节点）。
- 每个 RPC 必须有 `access`。handler 里不写鉴权 if。不建用户表。权限码先入 `permission_definition`，菜单不能发明新码。
- 禁止：收成 handler+repo；导出 `Permission` 字段去 getter；换 Casbin / 改用 `keyMatch2`；加 `lo`/`copier`/`mapstructure`；为策略同步加 outbox/MQ；手改 `*.pb.go` / `ent/` 生成文件 / `wire_gen.go`；预埋没有调用方的接口、表、API。
