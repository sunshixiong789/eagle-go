...
| 名词 | 是什么 | 在这个项目里的角色 |
|---|---|---|
| **Go module** | Go 的依赖管理机制，`go.mod`/`go.sum` 就是它的产物 | 类似 Java 的 Maven、Node 的 npm |
...
| **ent** | 一个"ORM"（对象关系映射），用 Go 代码描述数据库表结构（见 `ent/schema/`），自动生成增删改查代码，不用手写 SQL 拼接 | `ent/` 目录下 90% 以上的代码都是自动生成的，你不需要看懂，只需要知道改 `ent/schema/*.go` 然后重新生成即可 |
| **Kratos** | 一个 Go 微服务框架（B站开源），负责 HTTP/gRPC 服务器怎么启动、中间件怎么串、日志/链路追踪怎么接 | 相当于 Java 生态里 Spring Boot 的角色 |
