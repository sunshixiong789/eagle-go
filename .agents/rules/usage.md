...
分层理由见 [architecture.md](architecture.md)；写法约束见同目录其它规则。
...
边界和「为什么」见 [architecture.md](architecture.md)。从别的语言转过来可以先记这张表：
...
- [ ] 新 RPC 写了 `access`；需要权限的 `perm` 是三段且已进 `permission_definition`
- [ ] proto / `ent/schema` 改完已生成，表结构有 goose 迁移
- [ ] handler 里没鉴权 if，本库没存用户名密码
- [ ] `go test ./...` 能过
