...
加后端接口的顺序见 [docs/usage.md](../../docs/usage.md) 第 4 节，不可颠倒：proto（`access` + 需要时 `perm`）→ 新码写入 `permission_definition` → 生成 → 模块 domain/infrastructure/application/service → 新错误挂 `server.ErrorMapping` → 显式装配新 Service。
...
- 只有 admin 连接策略库；其他服务通过只读 `AuthorizationService.CheckPermission` 判定，失败时关闭。禁止资源服务直连权限表。
...
- 错误原因加在对应模块 API 包的 `error_reason.proto`，再挂 `ErrorMapping`。不要引入 `protoc-gen-go-errors`。
