...
加后端接口的顺序见 [docs/usage.md](../../docs/usage.md) 第 4 节，不可颠倒：proto（`access` + 需要时 `perm`）→ 新码写入 `permission_definition` → 生成 → 模块 domain/infrastructure/application/interfaces → 新错误挂 `server.ErrorMapping` → 显式装配新 Service。
